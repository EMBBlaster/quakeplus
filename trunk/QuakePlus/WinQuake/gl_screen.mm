...
void SCR_ScreenShot_f (void) 
{
	...
	int		i;
        extern bool	GL_SaveScreenshot (const char *);
	...
	if (i==100) 
	{
		Con_Printf ((char*)"SCR_ScreenShot_f: Couldn't create a PNG file\n"); 
		return;
 	}
...
//	buffer = malloc(glwidth*glheight*3 + 18);
	buffer = new char[glwidth*glheight*3 + 18];
        if (buffer != NULL)
        {
            memset (buffer, 0, 18);
...
//            free (buffer);
        ...
        if (GL_SaveScreenshot (checkname) == true)
            ...
            Con_Printf ((char*)"Failed to write %s\n", pcxname);
} 
