...
// Modified by: Martin Linklater (mslinklater@mac.com) [http://www.95percentchimp.co.uk]
//
// Quake‚Ñ¢ is copyrighted by id software		[http://www.idsoftware.com].
//
...
- (void) handleConsoleCommand: (NSScriptCommand *) theCommand
{
    ...
    if (myArguments != NULL)
    {
        ...
        if (myCommandList != NULL && [myCommandList isEqualToString:@""] == NO)
        {
            ...
            if ([[self delegate] hostInitialized] == YES)
            {
                Cbuf_AddText (va ((char*)"%s\n", [myCommandList cStringUsingEncoding:NSUTF8StringEncoding]));
            }
            else
            {
