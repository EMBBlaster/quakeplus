...
//
// Quake‚Äö√ë¬¢ is copyrighted by id software	[http://www.idsoftware.com].
//
// Version History:
...
//	       Obscure characters within the base pathname [like '‚àÜ√≠'...] are now allowed.
...
SInt	Sys_FileOpenRead (char *thePath, SInt *theHandle)
{
    ...
    *theHandle = myHandle;
    ...
    {
        Sys_Error ((char*)"Can\'t open file \"%s\", reason: \"%s\".", thePath, strerror (errno));
    }
...
SInt	Sys_FileOpenWrite (char *thePath)
{
    ...
    if (myHandle == -1)
    {
        Sys_Error ((char*)"Can\'t open file \"%s\" for writing, reason: \"%s\".", thePath, strerror(errno));
    }
...
void	Sys_mkdir (char *thePath)
{
    ...
    if (errno != EEXIST)
    {
        Sys_Error ((char*)"\"mkdir %s\" failed, reason: \"%s\".", thePath, strerror(errno));
    }
}
...
void	Sys_MakeCodeWriteable (UInt32 theStartAddress, UInt32 theLength)
{
    ...
    {
        Sys_Error ((char*)"Memory protection change failed!\n");
    }
}
...
{
	void* pSym = dlsym( RTLD_DEFAULT, theName );

	if( pSym == 0)
		Sys_Error ((char*)"Failed to import a required function!\n");
...
char *	Sys_GetClipboardData (void)
{
    ...
    if ([myPasteboardTypes containsObject: NSStringPboardType])
    {
        ...
        {
            return (strdup ([myClipboardString cStringUsingEncoding:NSUTF8StringEncoding]));
        }
    }
    return (NULL);
}
...
void	Sys_CheckForIDDirectory (void)
{
    ...
    {
        Sys_Error ((char*)"The mission pack has to be located within the same folder as the \"id1\" folder.");
    }
}
...
SInt	main (SInt theArgCount, const char **theArgValues)
{
    ...
    if (j) myParameters.memsize = (int) (atof (com_argv[j+1]) * 1024 * 1024);
//    if ((myParameters.membase = malloc (myParameters.memsize)) == NULL)
	if ((myParameters.membase = new char[ myParameters.memsize ] ) == NULL)
    {
