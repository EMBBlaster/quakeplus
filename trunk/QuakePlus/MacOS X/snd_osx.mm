...
bool SNDDMA_Init (void)
{
    ...
    Con_Printf ((char*)"Initializing CoreAudio...\n");
    ...
    {
        Con_Printf ((char*)"Audio init fails: Can\t get audio device.\n");
        return 0;
    }
    ...
    {
        Con_Printf ((char*)"Audio init fails: Unsupported audio device.\n");
        return 0;
    }
    ...
    {
        Con_Printf ((char*)"Audio init fails: Can't get audiobuffer.\n");
        return 0;
    }
    ...
    {
        Con_Printf ((char*)"Audio init: Audiobuffer size is not sufficient for optimized playback!\n");
    }
    if (sizeof (gSndBuffer) % gSndBufferByteCount != 0 ||
        sizeof (gSndBuffer) / gSndBufferByteCount < 2)
    {
        Con_Printf ((char*)"Audio init: Bad audiobuffer size!\n");
        return 0;
    }
    ...
    {
        Con_Printf ((char*)"Audio init fails.\n");
        return 0;
    }
    ...
    {
        Con_Printf ((char*)"Default Audio Device doesn't support Linear PCM!\n");
        return 0;
    }
    ...
    if (!COM_CheckParm ((char*)"-nosound"))
    {
        ...
        // add the sound FX IO:
		if (AudioDeviceCreateIOProcID (gSndDeviceID, SNDDMA_AudioIOProc, NULL, &outProcID))
        {
			gSndIOProcIsInstalled = NO;
            Con_Printf ((char*)"Audio init fails: Can\'t install IOProc.\n");
            return 0;
        }
        ...
        {
			gSndIOProcIsInstalled = NO;
            Con_Printf ((char*)"Audio init fails: Can\'t start audio.\n");
            return 0;
        }
        gSndIOProcIsInstalled = YES;
    }
    ...
    shm = (volatile dma_t *) Hunk_AllocName (sizeof (*shm), (char*)"shm");
    shm->splitbuffer = 0;
    shm->samplebits = 16;
    ...
    if (!COM_CheckParm ((char*)"-nosound"))
    {
        Con_Printf ((char*)"Sound Channels: %d\n", shm->channels);
        Con_Printf ((char*)"Sound sample bits: %d\n", shm->samplebits);
    }
...
void	SNDDMA_Shutdown (void)
{
    ...
    if (gSndIOProcIsInstalled == YES)
    {
        ...
        AudioDeviceDestroyIOProcID (gSndDeviceID, SNDDMA_AudioIOProc );
    }
}
