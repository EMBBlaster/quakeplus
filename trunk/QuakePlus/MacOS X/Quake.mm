...
#define	GAME_COMMAND		(char*)"-game"
#define	QUAKE_BASE_PATH		(char*)"."
...
bool gGraphicsWireframe;
NSTextView* g_consoleTextView;
...
{
	gGraphicsWireframe = [sender state];
}
...
//    myNewArgValues = (char **) malloc (sizeof(char *) * gSysArgCount);
    myNewArgValues = (char **) new char* [gSysArgCount];
    Sys_CheckMalloc( myNewArgValues );
...
//        gSysArgValues[i+1] = (char *) malloc (strlen (myCString) + 1);
        gSysArgValues[i+1] = (char *) new char[strlen (myCString) + 1];
        Sys_CheckMalloc( gSysArgValues[ i + 1 ] );
...
- (BOOL) application: (NSApplication *) theSender openFile: (NSString *) theFilePath
{
    ...
    if (theFilePath != NULL)
    {
        ...
        {
            Sys_Error ((char*)"The dragged item is not a valid file!");
        }
        if (myDirectory == NO)
        {
            Sys_Error ((char*)"The dragged item is not a folder!");
        }
        
        // prepare the new command line options:
//        myNewArgValues = (char **) malloc (sizeof(char *) * 3);
        myNewArgValues = (char **) new char* [3];
        Sys_CheckMalloc( myNewArgValues );
        gSysArgCount = 3;
        myNewArgValues[0] = gSysArgValues[0];
        gSysArgValues = myNewArgValues;
...
//        gSysArgValues[2] = (char *) malloc (strlen (myMod) + 1);
        gSysArgValues[2] = (char *) new char[strlen (myMod) + 1];
        Sys_CheckMalloc(gSysArgValues[2]);
...
- (void) applicationDidFinishLaunching: (NSNotification *) theNote
{
	@try
    {
		// setup up the info webview
		
		NSString	*welcomePage = [[NSBundle mainBundle] pathForResource: @"welcome" ofType: @"html"];
		
		[[newInfo mainFrame] loadRequest:[NSURLRequest requestWithURL:[NSURL fileURLWithPath:welcomePage]]];
		
		gGraphicsWireframe = [wireframeCheckbox state];
		g_consoleTextView = consoleTextView;
	...
	@catch (NSException* exception) {
		    NSLog(@"applicationDidFinishLaunching: Caught %@: %@", [exception name], [exception  reason]);
	}
}
...
- (void) awakeFromNib
{
    ...
    [self addToolbarItem:mToolbarItems 
			  identifier:SYS_ABOUT_TOOLBARITEM 
				   label:@"About" 
			paletteLabel:@"About"
                 toolTip:@"About Quake." 
				   image:@"about.tiff"
    ...
    [self addToolbarItem:mToolbarItems 
			  identifier:SYS_VIDEO_TOOLBARITEM 
				   label:@"Displays"
            paletteLabel:@"Displays" 
				 toolTip:@"Change display settings." 
				   image:@"displays.tiff"
                selector:@selector (showDisplaysView:)];
	
    [self addToolbarItem:mToolbarItems 
			  identifier:SYS_AUDIO_TOOLBARITEM 
				   label:@"Sound" 
			paletteLabel:@"Sound"
                 toolTip:@"Change sound settings." 
				   image:@"sound.tiff" 
				selector:@selector (showSoundView:)];
	
    [self addToolbarItem:mToolbarItems 
			  identifier:SYS_PARAM_TOOLBARITEM 
				   label:@"CLI" 
			paletteLabel:@"CLI"
                 toolTip:@"Set command-line parameters." 
				   image:@"cli.tiff"
				selector:@selector (showCLIView:)];
	
    [self addToolbarItem:mToolbarItems 
			  identifier:SYS_START_TOOLBARITEM 
				   label:@"Play" 
			paletteLabel:@"Play"
...
- (IBAction) buildResolutionList: (id) theSender
{
	NSLog(@"ACTION - buildResolutionList");
    ...
    Sys_CheckMalloc( myDisplayModes );
    mModeList = [[NSMutableArray alloc] init];
    Sys_CheckMalloc( mModeList );
...
- (IBAction) toggleColorsEnabled: (id) theSender
{
	NSLog(@"ACTION - toggleColorsEnabled");
...
- (IBAction) toggleParameterTextField: (id) theSender
{
	NSLog(@"ACTION - toggleParameterTextField");
...
- (IBAction) toggleMP3Playback: (id) theSender
{
	NSLog(@"ACTION - toggleMP3Playback");
...
//        [mediascanWindow makeKeyAndOrderFront: nil];
 //       [mediascanProgressIndicator startAnimation: self];
		...
		// This observer gets triggered once the threaded scanMediaThread: call has completed.
	...
	@catch (NSException* exception) {
		NSLog(@"newGame: Caught %@: %@", [exception name], [exception  reason]);
	}
}
                   ...
                   error: (NSString **) theError
{
    ...
    if ([myPasteboardTypes containsObject: NSStringPboardType])
    {
        ...
        {
            Cbuf_AddText (va ((char*)"connect %s\n", [myRequestedServer cStringUsingEncoding:NSUTF8StringEncoding]));
            return;
        }
    }
...
typedef boolean_t 	(*tCGDirectDisplayIDPtr)(CGDirectDisplayID);
...
- (void) buildDisplayList
{
    ...
    if (CGGetActiveDisplayList ( kVidMaxDisplays , gVidDisplayList, &gVidDisplayCount) != CGDisplayNoErr)
    {
        Sys_Error ((char*)"Can\'t build display list!");
    }
    
    // get some symbols (10.2+ only?):
    qCGDisplayIsMain = (tCGDirectDisplayIDPtr)Sys_GetProcAddress ("CGDisplayIsMain", NO);
    qCGDisplayIsBuiltin = (tCGDirectDisplayIDPtr)Sys_GetProcAddress ("CGDisplayIsBuiltin", NO);
...
- (void) scanMediaThread: (id) theSender
{
	@try
    {
		...
		// MSL - Why does this use the Distributed Notification Center ? The normal one seems to make it lock up or become unresponsive.
	...
	@catch (NSException* exception) {
		NSLog(@"scanMediaThread: Caught %@: %@", [exception name], [exception  reason]);
	}
}
...
//        [linkView setURLString: FRUITZ_OF_DOJO_URL];
	...
	@catch (NSException* exception) {
		NSLog(@"setupDialog: Caught %@: %@", [exception name], [exception  reason]);
	}
}
...
- (void) installFrameTimer
{
    if (mFrameTimer == NULL)
    {
        ...
        if (mFrameTimer == NULL)
        {
            Sys_Error ((char*)"Failed to install the renderer loop!");
        }
    }
}
...
//    [mediascanWindow close];
    ...
    j = COM_CheckParm ((char*)"-mem");
    if (j)
    {
        myParameters.memsize = (int) (atof (com_argv[j + 1]) * 1024 * 1024);
    }
    
//    myParameters.membase = malloc (myParameters.memsize);
    myParameters.membase = new char[myParameters.memsize];
    ...
    Con_Printf ((char*)"Quake++ for MacOS X -- Version %0.2f\n", kQuakePlusPlusVersion);
	Con_Printf((char*)"    ");
	Con_Printf((char*)"    Code by Martin Linklater\n");
	Con_Printf((char*)"        http://www.95percentchimp.co.uk\n");
	Con_Printf((char*)"    ");
	Con_Printf((char*)"    Based on the original Mac Port by Fruitz-of-Dojo\n");
	Con_Printf((char*)"        http://www.fruitz-of-dojo.de\n");
    ...
    if (mRequestedCommands != NULL)
    {
        while ([mRequestedCommands count] > 0)
        {
            ...
            Cbuf_AddText (va ((char*)"%s\n", [myCommand cStringUsingEncoding:NSUTF8StringEncoding]));
