...
cvar_t							vid_mode = { (char*)"vid_mode", (char*)"0", 0 },
								vid_redrawfull = { (char*)"vid_redrawfull", (char*)"0", 0 },
								vid_wait = { (char*)"vid_wait", (char*)"1", 1 },
								vid_overbright = { (char*)"gamma_overbright", (char*)"1", 1 },
								_vid_default_mode = { (char*)"_vid_default_mode", (char*)"0", 1 },
								_vid_default_blit_mode = { (char*)"_vid_default_blit_mode", (char*)"0", 1 },
								_windowed_mouse = { (char*)"_windowed_mouse",(char*)"0", 0 },
								gl_anisotropic = { (char*)"gl_anisotropic", (char*)"0", 1 },
								gl_fsaa = { (char*)"gl_fsaa", (char*)"0", 0 },
								gl_truform = { (char*)"gl_truform", (char*)"-1", 1 },
								gl_ztrick = { (char*)"gl_ztrick", (char*)"1" },
                                gl_multitexture = { (char*)"gl_multitexture", (char*)"0", 1 };
...
CGDirectDisplayID				gVidDisplayList[ kVidMaxDisplays ];
...
void	VID_SetGamma (void)
{
    ...
    if (myOldGamma != v_gamma.value || myOldOverbright != vid_overbright.value)
    {
        if (vid_overbright.value == 0.0f)
        {
            ...
            {
                Con_Printf ((char*)"Can\'t set the requested gamma value!\n");
            }
        }
        else
        {
...
void VID_SetWait (UInt32 theState)
{
    ...
    if((_CGError)CGLSetParameter (CGLGetCurrentContext (), kCGLCPSwapInterval, &params) == CGDisplayNoErr)
    {
        gGLVideoWait = vid_wait.value;
        if (theState == 0)
        {
            Con_Printf ((char*)"video wait successfully disabled!\n");
        }
        else
        {
            Con_Printf ((char*)"video wait successfully enabled!\n");
        }
    }
    else
    {
        vid_wait.value = gGLVideoWait;
        Con_Printf ((char*)"Error while trying to change video wait!\n");
    }    
}
...
BOOL	VID_SetDisplayMode (void)
{
    ...
    {
        Sys_Error ((char*)"Invalid list of active displays!");
    }
    ...
    if (gVidDisplayFullscreen == YES)
    {
        ...
        VSH_FadeGammaOut (gVidFadeAllDisplays, kVidFadeDuration );
        
        // capture display(s);
        if (VSH_CaptureDisplays (gVidFadeAllDisplays) == NO)
            Sys_Error ((char*)"Unable to capture display(s)!");
        
        // switch the display mode:
        if (CGDisplaySwitchToMode (gVidDisplayList[gVidDisplay], (CFDictionaryRef) gVidDisplayMode)
            != CGDisplayNoErr)
        {
            Sys_Error ((char*)"Unable to switch the displaymode!");
        }
    }
    
    // get the pixel format:
    if ((myPixelFormat = VID_CreateGLPixelFormat (gVidDisplayFullscreen)) == NULL)
        Sys_Error ((char*)"Unable to find a matching pixelformat. Please try other displaymode(s).");

    // initialize the OpenGL context:
    if (!(gGLContext = [[NSOpenGLContext alloc] initWithFormat: myPixelFormat shareContext: nil]))
        Sys_Error ((char*)"Unable to create an OpenGL context. Please try other displaymode(s).");

//	CGLError err;
	
//	err = CGLEnable( [gGLContext cglContext], kCGLCEMPEngine);
//	if( err != kCGLNoError )
//	{
//		NSLog(@"No MT OGL engine....");
//	}
    ...
    if (gVidDisplayFullscreen)
    {
        ...
        if (  (_CGError)CGLSetFullScreen ([gGLContext cglContext]) != CGDisplayNoErr)
            Sys_Error ((char*)"Unable to use the selected displaymode for fullscreen OpenGL.");
    ...
    else
    {
        ...
        {
            Sys_Error ((char*)"Unable to create content view!\n");
        }
        ...
        {
            Sys_Error ((char*)"Unabled to allocate the window buffer!\n");
        }
    }
...
void	VID_Init (unsigned char *thePalette)
{
    ...
    Cvar_RegisterVariable (&vid_overbright);
    ...
    if (VID_SetDisplayMode() == NO)
        Sys_Error ((char*)"Can\'t initialize video!");

    // setup console width according to display width:
    if ((i = COM_CheckParm((char*)"-conwidth")))
    ...
    if ((i = COM_CheckParm ((char*)"-conheight")))
    ...
    snprintf (myGLDir, MAX_OSPATH, (char*)"%s/glquake", com_gamedir);
    Sys_mkdir (myGLDir);
    ...
    Con_SafePrintf ((char*)"Video mode %dx%d initialized.\n", gGLDisplayWidth, gGLDisplayHeight);
    vid.recalc_refdef = 1;
}
...
void	VID_Shutdown (void)
{
    ...
    if (gVidDisplayFullscreen == YES)
    {
        if (gGLOriginalMode != NULL)
        {
            ...
            VSH_FadeGammaIn (gVidFadeAllDisplays, kVidFadeDuration );
...
void	VID_MenuDraw (void)
{
    ...
    myPicture = Draw_CachePic ((char*)"gfx/vidmodes.lmp");
    M_DrawPic ((320 - myPicture->width) / 2, 4, myPicture);
    
    // draw vid_wait option:
    M_Print( kVidFontWidth, myRow, (char*)"Video Sync:");
    if (vid_wait.value) M_Print ((39 - 2) * kVidFontWidth, myRow, (char*)"On");
        else M_Print ((39 - 3) * kVidFontWidth, myRow, (char*)"Off");
    if (gGLMenuLine == myRow) gGLMenuItem = VID_MENUITEM_WAIT;
    myRow += kVidFontHeight;

    // draw vid_overbright option:
    M_Print( kVidFontWidth, myRow, (char*)"Overbright Gamma:");
    if (vid_overbright.value) M_Print ((39 - 2) * kVidFontWidth, myRow, (char*)"On");
        else M_Print ((39 - 3) * kVidFontWidth, myRow, (char*)"Off");
    ...
    {
        myRow += kVidFontHeight;
        M_Print( kVidFontWidth, myRow, (char*)"FSAA:");
        if (gl_fsaa.value == 0.0f)
        {
             M_Print ((39 - 3) * kVidFontWidth, myRow, (char*)"Off");
        }
        else
        {
            snprintf (myString, 16, "%dx", (int) gl_fsaa.value);
            M_Print ((39 - strlen (myString)) * kVidFontWidth, myRow, myString);
        }
    ...
    {
        myRow += kVidFontHeight;
        M_Print( kVidFontWidth, myRow, (char*)"Anisotropic Texture Filtering:");
		
        if (gl_anisotropic.value)
		{
			M_Print ((39 - 2) * kVidFontWidth, myRow, (char*)"On");
		}
		else
		{
            M_Print ((39 - 3) * kVidFontWidth, myRow, (char*)"Off");
    ...
    {
        myRow += kVidFontHeight;
        M_Print( kVidFontWidth, myRow, (char*)"Multitexturing:");
    ...
    {
        myRow += kVidFontHeight;
        M_Print( kVidFontWidth, myRow, (char*)"ATI Truform Tesselation Level:");
    ...
    M_Print (4 * kVidFontWidth + 4, 36 + 23 * kVidFontHeight, (char*)"Video modes must be set at the");
    M_Print (11 * kVidFontWidth + 4, 36 + 24 * kVidFontHeight, (char*)"startup dialog!");
...
void	GL_CheckMultiTextureExtensions (void) 
{
    ...
    if (GL_ExtensionSupported ((char*)"GL_ARB_multitexture") == YES &&
        gl_luminace_lightmaps == YES &&
        !COM_CheckParm ((char*)"-nomtex"))
    {
        // attach symbols:
        qglSelectTextureSGIS = (void(*)(GLenum))Sys_GetProcAddress ("glActiveTextureARB", NO);
		qglMTexCoord2fSGIS = (void(*)(GLenum,GLfloat,GLfloat))Sys_GetProcAddress ("glMultiTexCoord2fARB", NO);
		...
		if (qglMTexCoord2fSGIS != NULL && qglSelectTextureSGIS != NULL)
        {
            ...
            {
                Con_Printf ((char*)"Found GL_ARB_multitexture...\n(%d texture units)\n", myMaxTextureUnits);
                gGLMultiTextureAvailable = YES;
            }
            else
            {
                Con_Printf ((char*)"Too less texture units (%d).\nGL_ARB_multitexture disabled.\n", myMaxTextureUnits);
                gGLMultiTextureAvailable = NO;
            }
		}
        else
        {
            Con_Printf ((char*)"Failed to look up symbol.\nGL_ARB_multitexture disabled.\n");
            gGLMultiTextureAvailable = NO;
        }
    }
    else
...
void	GL_CheckPalettedTexture (void)
{
    ...
    {
        gl_palettedtex = YES;
        Con_Printf ((char*)"Found GL_EXT_paletted_texture...\n");
    }
    else
    {
        gl_palettedtex = NO;
...
void	GL_CheckPNTrianglesExtensions (void)
{
    if (GL_ExtensionSupported ("GL_ATIX_pn_triangles") == YES)
    {
        ...
        gGLPNTrianglesiATIX = (void(*)(GLenum,GLint))Sys_GetProcAddress ("glPNTrianglesiATIX", NO);
        gGLPNTrianglesfATIX = (void(*)(GLenum,GLfloat))Sys_GetProcAddress ("glPNTrianglesfATIX", NO);

        // are the symbols present?
        if (gGLPNTrianglesiATIX != NULL && gGLPNTrianglesfATIX != NULL)
        {
            Con_Printf ((char*)"Found GL_ATIX_pn_triangles...\n");       
            gl_pntriangles = YES;
        }
        else
        {
            Con_Printf ((char*)"Failed to look up symbol. GL_ATIX_pn_triangles disabled.\n");
            gl_pntriangles = NO;
        }
    }
    else
...
BOOL	GL_CheckARBMultisampleExtension (CGDirectDisplayID theDisplay)
{
    ...
    if ((_CGError)myError == kCGErrorSuccess)
    {
        ...
        for (myIndex = 0; myIndex < myCount; myIndex++)
        {
            ...
            if ((_CGError)myError == kCGErrorSuccess && mySampleBuffers > 0)
            {
                ...
                if ((_CGError)myError == kCGErrorSuccess && mySamples > myMaxSamples)
                {
...
void	GL_CheckSwitchFSAAOnTheFly (void)
{
    ...
    {
        Con_Printf ((char*)"Found ATI FSAA...\n");
...
{
    if (GL_ExtensionSupported ((char*)"GL_EXT_texture_filter_anisotropic") == YES)
    {
        Con_Printf ((char*)"Found GL_EXT_texture_filter_anisotropic...\n");
...
void	GL_CheckLuminanceLightmaps (void)
{
    ...
    {
        gl_luminace_lightmaps = YES;
        Con_Printf ((char*)"Found MacOS X v10.2 or later. Using luminance lightmaps...\n");
    }
    else
    {
        gl_luminace_lightmaps = NO;
        Con_Printf ((char*)"Found MacOS X v10.1 or earlier.  Using RGBA lightmaps...\n");
    }
}
                            ...
                            GLenum theType)
{
    ...
    {
        Sys_Error ((char*)"Out of texture RAM. Please try a lower resolution and/or depth!");
    }
}
...
bool GL_SaveScreenshot (const char *theFilename)
{
...
void	GL_SetFSAA (UInt32 theFSAALevel)
{
    ...
    {
        Cvar_SetValue (gl_fsaa.name, gGLFSAALevel);
        Con_Printf ((char*)"Invalid FSAA level, accepted values are 0, 4 or 8!\n");
        return;
    }
    ...
    if (gl_fsaaavailable == NO)
    {
        ...
        if (theFSAALevel != 0)
        {
            Con_Printf ((char*)"FSAA not supported with the current graphics board!\n");
            Cvar_SetValue (gl_fsaa.name, gGLFSAALevel);
        }
        return;
    }
    ...
    // set the level:
    [gGLContext makeCurrentContext];
    if ((_CGError)CGLSetParameter (CGLGetCurrentContext (), (CGLContextParameter)VID_ATI_FSAA_LEVEL, &myFSAALevel) == CGDisplayNoErr)
    {
        gGLFSAALevel = theFSAALevel;
        Con_Printf ((char*)"FSAA level set to: %d!\n", theFSAALevel);
    }
    else
    {
        Con_Printf ((char*)"Error while trying to set the new FSAA Level!\n");
    }
    Cvar_SetValue (gl_fsaa.name, gGLFSAALevel);
}
...
void	GL_SetPNTriangles (SInt32 thePNTriangleLevel)
{
    ...
    if (gl_pntriangles == NO)
    {
        ...
        {
            Con_Printf ((char*)"pntriangles not supported with the current graphics board!\n");
        }
    ...
    if (thePNTriangleLevel >= 0)
    {
        if (thePNTriangleLevel > 7)
        {
            ...
            Con_Printf ((char*)"Clamping to max. pntriangle level 7!\n");
        }
        ...
        Con_Printf ((char*)"Truform enabled, current tesselation level: %d!\n", thePNTriangleLevel);
    }
    else
    {
        ...
        Con_Printf ((char*)"Truform disabled!\n");
    }
...
void	GL_SetTextureFilterAnisotropic (UInt32 theState)
{
    ...
    if (gl_texturefilteranisotropic == NO)
    {
        ...
        if (theState != 0)
        {
            Con_Printf ((char*)"Anisotropic tetxure filtering not supported with the current graphics board!\n");
        }
        return;
    }
...
void	GL_SetMultiTexture (UInt32 theState)
{
    ...
    if (gGLMultiTextureAvailable == YES)
    {
        ...
        if (gl_mtexable == YES)
        {
            Con_Printf ((char*)"Multitexturing enabled!\n");
        }
        else
        {
            Con_Printf ((char*)"Multitexturing disabled!\n");
        }
    }
    else
    {
        Con_Printf ((char*)"Multitexturing not available!\n");
    }
}
...
void	GL_BeginRendering (SInt *x, SInt *y, SInt *width, SInt *height)
{
    ...
    *height = gGLDisplayHeight;
	...
	// Do some on-the fly state setting up...
	if( gGraphicsWireframe )
