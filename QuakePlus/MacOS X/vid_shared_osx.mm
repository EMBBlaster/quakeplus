...
static CGDirectDisplayID	gVshCapturedDisplayList[ kVidMaxDisplays ];
...
BOOL	VSH_CaptureDisplays (BOOL theCaptureAllDisplays)
{
    ...
    myError = CGGetActiveDisplayList( kVidMaxDisplays, gVshCapturedDisplayList, &gVshCapturedDisplayCount);
...
//    gVshOriginalGamma = (vid_gamma_t*)malloc (sizeof (vid_gamma_t) * gVidDisplayCount);
    gVshOriginalGamma = (vid_gamma_t*)new char [sizeof (vid_gamma_t) * gVidDisplayCount];
    if (gVshOriginalGamma == NULL)
    {
        return (NO);
    }
...
//            free (gVshOriginalGamma);
    ...
    {
//        free (gVshGammaTable);
        delete [] gVshGammaTable;
        gVshGammaTable = NULL;
    }

    // required for our second gamma method:
//    gVshGammaTable = (vid_gammatable_t *) malloc (sizeof (vid_gammatable_t));
    gVshGammaTable = (vid_gammatable_t *)new char[ sizeof (vid_gammatable_t) ];
    if (gVshGammaTable != NULL)
    {
        myError = CGGetDisplayTransferByTable (gVidDisplayList[gVidDisplay], kVidGammaTableSize,
