	...
	switch (Cmd_Argc ())
	{
		case 1:
			if (((struct in_addr *)&banAddr)->s_addr)
			{
				...
				print((char*)"Banning %s [%s]\n", addrStr, maskStr);
			}
			else
				print((char*)"Banning not active\n");
			break;

		case 2:
			if (strcasecmp(Cmd_Argv(1), (char*)"off") == 0)
				banAddr = 0x00000000;
			else
		...
		default:
			print((char*)"BAN ip_address [mask]\n");
			break;
	}
}
#endif
...
#ifdef DEBUG
	if (data->cursize == 0)
		Sys_Error((char*)"Datagram_SendMessage: zero length message\n");

	if (data->cursize > NET_MAXMESSAGE)
		Sys_Error((char*)"Datagram_SendMessage: message too big %u\n", data->cursize);

	if (sock->canSend == false)
		Sys_Error((char*)"SendMessage: called with canSend == false\n");
#endif
...
//		continue;
		...
		{
#ifdef DEBUG
			Con_DPrintf((char*)"Forged packet received\n");
			Con_DPrintf((char*)"Expected: %s\n", StrAddr (&sock->addr));
			Con_DPrintf((char*)"Received: %s\n", StrAddr (&readaddr));
#endif
			continue;
		}
...
void NET_Stats_f (void)
{
	...
	else if (strcmp(Cmd_Argv(1), (char*)"*") == 0)
	{
...
JustDoIt:
	...
	MSG_WriteLong(&net_message, 0);
	MSG_WriteByte(&net_message, CCREQ_RULE_INFO);
	MSG_WriteString(&net_message, (char*)"");
	*((int *)net_message.data) = BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
...
#ifdef BAN_TEST
	Cmd_AddCommand ((char*)"ban", NET_Ban_f);
#endif
	Cmd_AddCommand ((char*)"test", Test_f);
	Cmd_AddCommand ((char*)"test2", Test2_f);
...
static qsocket_t *_Datagram_CheckNewConnections (void)
{
	...
	{
		if (strcmp(MSG_ReadString(), (char*)"QUAKE") != 0)
			return NULL;
	...
	if (MSG_ReadByte() != NET_PROTOCOL_VERSION)
	{
		...
		MSG_WriteString(&net_message, (char*)"Incompatible version.\n");
		*((int *)net_message.data) = BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
		dfunc.Write (acceptsock, net_message.data, net_message.cursize, &clientaddr);
		SZ_Clear(&net_message);
		return NULL;
	...
	if (clientaddr.sa_family == AF_INET)
	{
		...
		if ((testAddr & banMask) == banAddr)
		{
			...
			MSG_WriteLong(&net_message, 0);
			MSG_WriteByte(&net_message, CCREP_REJECT);
			MSG_WriteString(&net_message, (char*)"You have been banned.\n");
			*((int *)net_message.data) = BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
			dfunc.Write (acceptsock, net_message.data, net_message.cursize, &clientaddr);
			SZ_Clear(&net_message);
			return NULL;
	...
	if (sock == NULL)
	{
		...
		MSG_WriteLong(&net_message, 0);
		MSG_WriteByte(&net_message, CCREP_REJECT);
		MSG_WriteString(&net_message, (char*)"Server is full.\n");
		*((int *)net_message.data) = BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
		dfunc.Write (acceptsock, net_message.data, net_message.cursize, &clientaddr);
		SZ_Clear(&net_message);
		return NULL;
		...
		do
		{
			ret = dfunc.Read (newsock, net_message.data, net_message.maxsize, &readaddr);
                        ...
                        if (sfunc.AddrCompare(&readaddr, &sendaddr) != 0)
                        {
                            ...
                            {
                                Con_Printf ((char*)"Patching server address!\n");
                                sendaddr = readaddr;
                            }
                        }
