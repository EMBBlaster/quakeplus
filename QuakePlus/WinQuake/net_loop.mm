...
void Loop_SearchForHosts (bool xmit)
{
	...
	if (strcmp(hostname.string, (char*)"UNNAMED") == 0)
		strcpy(hostcache[0].name, (char*)"local");
	else
	...
	strcpy(hostcache[0].cname, (char*)"local");
}


qsocket_t *Loop_Connect (char *host)
{
	if (strcmp(host,(char*)"local") != 0)
		return NULL;
	...
	if (!loop_client)
	{
		...
		{
			Con_Printf((char*)"Loop_Connect: no qsocket available\n");
			return NULL;
		}
		strcpy (loop_client->address, (char*)"localhost");
	}
	...
	if (!loop_server)
	{
		...
		strcpy (loop_server->address, (char*)"LOCAL");
	}
